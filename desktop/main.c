...
/*本程序版本相关的宏定义*/
#define MAIN_VERSION_MAJOR 1 /*主版本号*/
#define MAIN_VERSION_MINOR 2 /*次版本号*/
...
/*文件相关的宏定义*/
#define APP_DIR_NAM "app"              /*应用目录名称*/
#define APP_LST_FIL APP_DIR_NAM ".lst" /*应用列表的文件名称*/
#define APP_CNT_FIL APP_DIR_NAM ".cnt" /*应用数量的文件名称*/

/*全局变量*/
char slf_dir[DIR_MAX_LEN] = {0};
char fil_sys_dir[DIR_MAX_LEN] = {0};
char app_dir[DIR_MAX_LEN] = {0};
...
static void get_app_dir(void)
{
    ...
    if (256 == system(cmd_buf))
    {
        ...
        sprintf(tmp_buf,
                ...
                slf_dir); /*生成创建指定目录的指令*/
#if DEBUG
        LV_LOG_USER("cmd:%s", tmp_buf); /*输出执行的命令*/
#endif
        system(tmp_buf); /*创建app目录*/
    }
#if DEBUG
    LV_LOG_USER("cmd:%s", cmd_buf); /*输出执行的命令*/
#endif
    system(cmd_buf); /*执行命令*/
}
...
static void lod_app(void)
{
    ...
    sprintf(cmd_buf,
            "%s/%s",
            app_dir,
            APP_CNT_FIL);                /*生成APP_CNT_FIL文件的绝对路径*/
