...
static void app_btn_pre_evt_cb(lv_event_t *evt)
{
    ...
    lv_obj_t *evt_app_lab = lv_obj_get_child(evt_app_btn, 0); /*获取应用的标签对象*/
    lv_obj_add_state(evt_app_lab, LV_STATE_PRESSED);          /*为标签添加按压状态*/
    lv_style_transition_dsc_init(
        &app_btn_def_sty_tst_dsc,
        app_btn_def_sty_pop,
        APP_BTN_DEF_ANI,
...
static void app_btn_siz_evt_cb(lv_event_t *evt)
{
    ...
    {
#if DEBUG
        LV_LOG_USER("evt_data:%s", (char *)evt->user_data); /*输出事件传递的数据*/
#endif
        sprintf(cmd_buf,
                "%s/%s/%s",
                slf_dir,
                (char *)evt->user_data, "exe");                   /*生成运行指定程序的命令*/
        system(cmd_buf);                                          /*执行命令*/
        lv_event_send(evt_app_btn, LV_EVENT_SHORT_CLICKED, NULL); /*程序触发短点击事件*/
    }
}

/*依据当前排列的位置换算需要放置的屏幕坐标*/
...
/*初始化应用列表中的应用*/
void app_int(App *const lst, const uint8_t cnt)
{
    ...
    for (uint8_t idx = 0; idx < cnt; idx++)
    {
        ...
        lv_obj_set_style_bg_img_src(tmp_obj, &lst[idx].icn[0], LV_STATE_DEFAULT); /*设置背景图片*/
